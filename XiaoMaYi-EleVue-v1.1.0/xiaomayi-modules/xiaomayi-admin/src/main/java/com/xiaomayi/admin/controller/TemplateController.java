...
public class TemplateController {
    ...
    @GetMapping("/page")
    public R page(TemplatePageDTO templatePageDTO) {
        return R.ok(templateService.page(templatePageDTO));
    }
    ...
    @RequestLog(title = "添加模板", type = RequestType.INSERT)
    @PreAuthorize("@pms.hasAuthority('sys:template:add')")
    @PostMapping("/add")
    public R add(@RequestBody @Validated TemplateAddDTO templateAddDTO) {
        return templateService.add(templateAddDTO);
    }
     ...
     * @return 返回结果
     */
    @Operation(summary = "更新模板", description = "更新模板")
    @RequestLog(title = "更新模板", type = RequestType.UPDATE)
    @PreAuthorize("@pms.hasAuthority('sys:template:update')")
    @PutMapping("/update")
    public R update(@RequestBody @Validated TemplateUpdateDTO templateUpdateDTO) {
        return templateService.update(templateUpdateDTO);
    }
    ...
    @RequestLog(title = "删除模板", type = RequestType.DELETE)
    @PreAuthorize("@pms.hasAuthority('sys:template:delete')")
    ...
    @RequestLog(title = "批量删除模板", type = RequestType.BATCH_DELETE)
    @PreAuthorize("@pms.hasAuthority('sys:template:batchDelete')")
