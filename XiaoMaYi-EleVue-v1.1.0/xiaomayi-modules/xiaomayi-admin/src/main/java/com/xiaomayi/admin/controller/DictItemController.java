...
public class DictItemController {
    ...
    @GetMapping("/page")
    public R page(DictItemPageDTO dictItemPageDTO) {
        return R.ok(dictItemService.page(dictItemPageDTO));
    }
    ...
    @GetMapping("/list")
    public R getList(DictItemListDTO dictItemListDTO) {
        return R.ok(dictItemService.getList(dictItemListDTO));
    }
    ...
    public R getDetail(@PathVariable("id") Integer id) {
        return R.ok(dictItemService.getDetail(id));
    }

    /**
     * 添加字典数据
     * 特别备注：@CacheEvict是用来标注在需要清除缓存元素的方法或类上的，allEntries参数值图尔表示清除所有元素，默认false
     *
     * @param dictItemAddDTO 参数
     * @return 返回结果
     */
    @Operation(summary = "添加字典数据", description = "添加字典数据")
    @RequestLog(title = "添加字典数据", type = RequestType.INSERT)
    @PreAuthorize("@pms.hasAuthority('sys:dict:add')")
    @CacheEvict(value = CacheConstant.SYS_DICT_KEY, allEntries = true)
    @PostMapping("/add")
    public R add(@RequestBody @Validated DictItemAddDTO dictItemAddDTO) {
        return dictItemService.add(dictItemAddDTO);
    }
    ...
    @RequestLog(title = "更新字典数据", type = RequestType.UPDATE)
    @PreAuthorize("@pms.hasAuthority('sys:dict:update')")
//    @CachePut(value = CacheConstant.SYS_DICT_KEY, key = "#dictItemUpdateDTO.dictCode")
    @CacheEvict(value = CacheConstant.SYS_DICT_KEY, allEntries = true)
//    @CacheEvict(value = CacheConstant.SYS_DICT_KEY, key = "#dictItemUpdateDTO.dictCode")
    @PutMapping("/update")
    public R update(@RequestBody @Validated DictItemUpdateDTO dictItemUpdateDTO) {
        return dictItemService.update(dictItemUpdateDTO);
    }
    ...
    @RequestLog(title = "批量删除字典数据", type = RequestType.BATCH_DELETE)
     ...
     * @return 返回结果
     */
    @Operation(summary = "根据编码获取字典数据列表", description = "根据编码获取字典数据列表")
    @GetMapping("/getDictItemList/{dictCode}")
//    @Cacheable(value = CacheConstant.SYS_DICT_KEY, key = "#dictCode", unless = "#result.data.isEmpty()")
    public R<List<DictItem>> getDictItemList(@PathVariable String dictCode) {
        return R.ok(dictItemService.getDictItemList(dictCode));
    }
