// +----------------------------------------------------------------------
// | 小蚂蚁云企业级开发框架 [ 赋能开发者，助力企业发展 ]
// +----------------------------------------------------------------------
// | 版权所有 2022~2024 小蚂蚁团队
// +----------------------------------------------------------------------
// | Licensed Apache-2.0 【小蚂蚁云】并不是自由软件，未经许可禁止去掉相关版权
// +----------------------------------------------------------------------
// | 官方网站: https://www.xiaomayicloud.com
// +----------------------------------------------------------------------
// | 软件作者: @小蚂蚁团队 团队荣誉出品
// +----------------------------------------------------------------------
// | 版权和免责声明:
// | 本团队对该软件框架产品拥有知识产权（包括但不限于商标权、专利权、著作权、商业秘密等）
// | 均受到相关法律法规的保护，任何个人、组织和单位不得在未经本团队书面授权的情况下对所授权
// | 软件框架产品本身申请相关的知识产权，被授权主体务必妥善保管官方所授权的软件产品源码，禁
// | 止以任何形式对外泄露(包括但不限于分享、开源、网络平台),禁止用于任何违法、侵害他人合法
// | 权益等恶意的行为，禁止用于任何违反我国法律法规的一切项目研发，任何个人、组织和单位用于
// | 项目研发而产生的任何意外、疏忽、合约毁坏、诽谤、版权或知识产权侵犯及其造成的损失 (包括
// | 但不限于直接、间接、附带或衍生的损失等)，本团队不承担任何法律责任，本软件框架禁止任何
// | 单位、组织、个人用于任何违法、侵害他人合法利益等恶意的行为，如有发现违规、违法的犯罪行
// | 为，本团队将无条件配合公安机关调查取证同时保留一切以法律手段起诉的权利，本软件框架只能
// | 用于公司和个人内部的法律所允许的合法合规的软件产品研发，详细声明内容请阅读《框架免责声
// | 明》附件；
// +----------------------------------------------------------------------

package com.xiaomayi.admin.controller.demo;


import com.baomidou.dynamic.datasource.annotation.DS;
import com.xiaomayi.admin.entity.User3;
import com.xiaomayi.admin.service.IUser3Service;
import com.xiaomayi.core.utils.R;
import com.xiaomayi.idempotent.annotation.RepeatSubmit;
import com.xiaomayi.logger.annotation.RequestLog;
import com.xiaomayi.logger.enums.RequestType;
import com.xiaomayi.oss.config.AliyunOssConfig;
import com.xiaomayi.security.annotation.RequestIgnore;
import com.xiaomayi.security.utils.SecurityUtils;
import com.xiaomayi.sms.config.AliyunSmsConfig;
import com.xiaomayi.system.dto.user.UserAddDTO;
import com.xiaomayi.system.dto.user.UserUpdateDTO;
import lombok.AllArgsConstructor;
import org.springframework.web.bind.annotation.*;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * <p>
 * 用户表 前端控制器
 * </p>
 *
 * @author 小蚂蚁团队
 * @since 2024-05-26
 */
@RequestIgnore
@DS("demo")
@RestController
@RequestMapping("/user3")
@AllArgsConstructor
public class User3Controller {

    private final IUser3Service user3Service;

    /* 方法注解 */
//    @DataSource(value = DataSourceType.DEMO)
    @GetMapping("/list")
    public List<User3> list() {
        List<User3> list = user3Service.getUserList();
        return list;
    }

    @RequestLog(title = "添加用户", type = RequestType.INSERT, exclude = {"gender"})
    @PostMapping("/add")
    public R add(@RequestBody UserAddDTO userAddDTO) {
        return R.ok();
    }

    /**
     * 忽略注解测试
     *
     * @param userUpdateDTO
     * @return
     */
    @RequestIgnore
    @PostMapping("/update")
    public R update(@RequestBody UserUpdateDTO userUpdateDTO) {
        System.out.println(SecurityUtils.getUsername());
        return R.ok();
    }

    /**
     * 忽略注解测试
     *
     * @return
     */
    @RequestIgnore
    @GetMapping("/info/{id}")
    public R update(@PathVariable("id") Integer id) {
        String bucketName = AliyunOssConfig.getBucketName();
        String signName = AliyunSmsConfig.getSignName();
        // 发送短信
//        JSONObject object = new JSONObject();
//        object.set("code", "123456");
        Map<String, String> map = new HashMap<>();
        map.put("code", "123456");
//        AliyunSmsUtil.sendSms("15295504151", JSON.toJSONString(map));
        return R.ok();
    }

    /**
     * 幂等性验证
     *
     * @return 返回结果
     */
    @RepeatSubmit
    @PostMapping("/idem")
    public R idem(@RequestBody Map<String, String> map) {
        return R.ok();
    }

}
