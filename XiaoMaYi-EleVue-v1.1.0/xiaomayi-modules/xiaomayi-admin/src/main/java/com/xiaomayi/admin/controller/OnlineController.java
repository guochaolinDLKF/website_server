...
public class OnlineController {
     ...
     * @return 返回结果
     */
    @Operation(summary = "获取在线用户列表", description = "获取在线用户列表")
    @PreAuthorize("@pms.hasAuthority('sys:online:list')")
    @GetMapping("/list")
    public R list(OnlineListDTO onlineListDTO) {
        return onlineService.list(onlineListDTO);
    }
    ...
    @RequestLog(title = "强制退出", type = RequestType.FORCE)
    @PreAuthorize("@pms.hasAuthority('sys:online:logout')")
    @DeleteMapping("/logout/{tokenId}")
    public R logout(@PathVariable String tokenId) {
        return onlineService.logout(tokenId);
    }
