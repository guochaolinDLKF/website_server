...
@Tag(name = "定时任务日志", description = "定时任务日志")
@AllArgsConstructor
public class JobLogController {
    ...
    @GetMapping("/page")
    public R page(@Validated JobLogPageDTO jobLogPageDTO) {
        return R.ok(jobLogService.page(jobLogPageDTO));
    }
    ...
    public R getDetail(@PathVariable("id") Integer id) {
        return R.ok(jobLogService.getDetail(id));
    }
     ...
     * @return 返回结果
     */
    @Operation(summary = "批量删除定时任务日志", description = "批量删除定时任务日志")
    @RequestLog(title = "批量删除定时任务日志", type = RequestType.BATCH_DELETE)
