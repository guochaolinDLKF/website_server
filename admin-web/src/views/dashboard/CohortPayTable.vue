<template>
  <el-card class="page-card cp-card" shadow="never">
    ...
    <div class="cp-tabs">
      ...
      <!-- 指标：留存 / 流失 -->
      <el-popover ref="metricPopRef" trigger="click" placement="bottom-start" :width="120" popper-class="gm-pop">
        <template #reference>
          <span class="cp-tab cp-gran">{{ METRIC_LABEL[metric] }}<i class="cp-caret">▾</i></span>
        </template>
        <div class="gm-menu">
          <div class="gm-row" :class="{ active: metric === 'retain' }" @click="pickMetric('retain')">留存</div>
          <div class="gm-row" :class="{ active: metric === 'churn' }" @click="pickMetric('churn')">流失</div>
        </div>
      </el-popover>
      <span class="cp-sep">|</span>
    ...
    <div class="cp-body" v-loading="loading">
        ...
        <table class="cp-table">
          <thead>
            <tr>
              ...
              <th class="cp-col-reg">账号注册用户数</th>
          ...
          <tbody>
            <tr v-if="data.summary" class="cp-row-summary">
              ...
              <td v-for="(c, i) in data.summary.cells" :key="i" :style="cellStyle(c)">
                <template v-if="c.cum !== null && c.cum !== undefined">
                  <div class="cp-cell-cum">{{ fmtInt(cellMain(c, data.summary.reg)) }}</div>
                  <div class="cp-cell-rate">{{ fmtPct(cellRate(c)) }}</div>
                  <div class="cp-cell-day">{{ fmtInt(c.day) }}</div>
                </template>
            ...
            <tr v-for="row in data.rows" :key="row.date">
              ...
              <td v-for="(c, i) in row.cells" :key="i" :style="cellStyle(c)">
                <template v-if="c.cum !== null && c.cum !== undefined">
                  <div class="cp-cell-cum">{{ fmtInt(cellMain(c, row.reg)) }}</div>
                  <div class="cp-cell-rate">{{ fmtPct(cellRate(c)) }}</div>
                  <div class="cp-cell-day">{{ fmtInt(c.day) }}</div>
                </template>
                <span v-else class="cp-empty">—</span>
      ...
      <div class="cp-legend">{{ legendText }}</div>
    </div>
  </el-card>
</template>
...
const props = defineProps({
  title: { type: String, default: '注册后阶段累计付费人数' },
  tip: { type: String, default: '' },
  maxStage: { type: Number, default: 7 },
...
const METRIC_LABEL = { retain: '留存', churn: '流失' }

const loading = ref(false)
const data = reactive({ stages: [], rows: [], summary: null })
const dateRangeRef = ref()
const granPopRef = ref()
const metricPopRef = ref()
const rangeStart = ref('')
const rangeEnd = ref('')
const dim = ref('day')
const metric = ref('retain') // retain=留存(累计付费) | churn=流失(累计未付费)

const legendText = computed(() =>
  metric.value === 'churn'
    ? '每格三行：累计未付费人数 / 累计流失率 / 当日付费人数（流失=注册数−累计付费）'
    : '每格三行：累计付费人数 / 累计付费率 / 当日付费人数'
)
...
// 主指标人数：留存=累计付费，流失=注册数−累计付费
function cellMain(c, reg) {
  if (!c || c.cum === null || c.cum === undefined) return null
  return metric.value === 'churn' ? Math.max(0, Number(reg || 0) - Number(c.cum)) : Number(c.cum)
}
// 主指标率：留存=累计付费率，流失=100−付费率
function cellRate(c) {
  if (!c || c.rate === null || c.rate === undefined) return null
  return metric.value === 'churn' ? Number((100 - Number(c.rate)).toFixed(2)) : Number(c.rate)
}
// 蓝色热力：按当前指标率深浅着色
...
function pickMetric(m) {
  metricPopRef.value && metricPopRef.value.hide()
  metric.value = m // 仅切换展示口径，无需重新请求
}
