...
REM 配置信息 - 部署前请修改为实际值
set SERVER_IP=YOUR_ARRAIGNMENT_SERVER_IP
set SERVER_PORT=22
set SERVER_USER=root
set SERVER_PASSWORD=YOUR_PASSWORD
set CONTAINER_NAME=server-arraignment
set IMAGE_NAME=server:arraignment
...
echo === 启动应用容器 ===
ssh -o StrictHostKeyChecking=no -p %SERVER_PORT% %SERVER_USER%@%SERVER_IP% "docker run -d --name %CONTAINER_NAME% -p 8000:8000 -v /home/fortune-telling/logs:/home/fortune-telling/logs --restart unless-stopped -e SPRING_PROFILES_ACTIVE=arraignment %IMAGE_NAME%"
