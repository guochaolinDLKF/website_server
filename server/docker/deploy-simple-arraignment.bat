...
chcp 65001 >nul
echo ========================================
echo  Deploy Script (ARRAIGNMENT)
echo ========================================
echo.
...
set SERVER_IP=%ARRAIGNMENT_SERVER_IP%
set SERVER_PORT=%ARRAIGNMENT_SERVER_PORT%
set SERVER_USER=%ARRAIGNMENT_SERVER_USER%
set SERVER_PASSWORD=%ARRAIGNMENT_SERVER_PASSWORD%
set CONTAINER_NAME=website_server-arraignment
set IMAGE_NAME=website_server:arraignment
set JAR_NAME=website_server-1.0.0.jar

if "%SERVER_IP%"=="" (
    echo [ERROR] ARRAIGNMENT_SERVER_IP is empty! Check deploy-config.bat
    pause
    exit /b 1
)
...
ssh -o StrictHostKeyChecking=no -p %SERVER_PORT% %SERVER_USER%@%SERVER_IP% "docker run -d --name %CONTAINER_NAME% -p 8660:8660 -v /home/website_server/logs:/home/website_server/logs --restart unless-stopped -e SPRING_PROFILES_ACTIVE=arraignment %IMAGE_NAME%"
