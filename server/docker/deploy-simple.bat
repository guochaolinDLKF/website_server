...
set IMAGE_NAME=server:latest
set JAR_NAME=website_server-1.0.0.jar
set SPRING_PROFILE=local-dev
...
echo === 启动应用容器 ===
ssh -o StrictHostKeyChecking=no -p %SERVER_PORT% %SERVER_USER%@%SERVER_IP% "docker run -d --name %CONTAINER_NAME% -p 8000:8000 -v /home/fortune-telling/logs:/home/fortune-telling/logs --restart unless-stopped -e SPRING_PROFILES_ACTIVE=%SPRING_PROFILE% %IMAGE_NAME%"
